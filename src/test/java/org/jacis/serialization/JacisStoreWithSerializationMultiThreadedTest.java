...
public class JacisStoreWithSerializationMultiThreadedTest {
  ...
  private void checkSimpleMultiThreadedAccess() {
    JacisStore<String, TestObject> store = new JacisTestHelper().createTestStoreWithSerialization();
    String testObjName = "TST";
    store.getContainer().withLocalTx(() -> store.update(testObjName, new TestObject(testObjName, 0).setStrValue("0")));
      ...
      threads[i] = new Thread("TestThread-" + i) {
        ...
        public void run() {
          try {
            ...
            log.info("interrupted thread {}: {}", Thread.currentThread().getName(), e, e);
